...
const getWeatherIcon = (desc?: string) => {
  ...
  if (safeDesc.includes("rain"))
    return (
      <CloudRain
        className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 text-[#0038A8] opacity-80"
        strokeWidth={1}
      />
    );
  if (safeDesc.includes("cloud"))
    return (
      <Cloud
        className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 text-muted-foreground opacity-80"
        strokeWidth={1}
      />
    );
  if (safeDesc.includes("thunder"))
    return (
      <CloudLightning
        className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 text-[#CE1126] opacity-80"
        strokeWidth={1}
      />
    );
  return (
    <Sun
      className="h-16 w-16 md:h-20 md:w-20 lg:h-24 lg:w-24 text-[#FCD116] opacity-80"
      strokeWidth={1}
    />
  );
};
...
export default function CurrentWeather() {
  ...
  const displayHigh =
    unit === "C" ? Math.round(high) : Math.round((high * 9) / 5 + 32);
  const displayLow =
    unit === "C" ? Math.round(low) : Math.round((low * 9) / 5 + 32);

  const description =
    weatherDetails?.description || current?.condition?.text || "Clear";
    ...
    <div className="flex flex-col justify-between h-full w-full relative z-10">
      ...
      <div className="relative z-10 mt-auto pt-2 pb-4 md:pb-6">
        <div className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold tracking-tighter text-foreground font-sans leading-none">
          {displayTemp}°
          <span className="text-xl md:text-2xl lg:text-3xl text-muted-foreground ml-1">
            {unit}
          </span>
        </div>
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center p-2 md:p-3 xl:p-4 rounded-xl md:rounded-2xl bg-muted/30 border border-border/40 hover:bg-muted/50 transition-colors min-w-0">
      <div className="flex items-center justify-center gap-1.5 md:gap-2 mb-1.5 md:mb-2 w-full">
        <Icon
          className={`w-3 h-3 md:w-4 md:h-4 shrink-0 ${themeColor}`}
          strokeWidth={2.5}
        />
        <span className="text-[9px] md:text-[10px] font-bold text-muted-foreground uppercase tracking-widest leading-tight truncate">
          {label}
        </span>
      </div>
      <span className="text-sm md:text-lg lg:text-base xl:text-xl font-bold text-foreground leading-none whitespace-nowrap tracking-tight">
        {value}
      </span>
    </div>
  );
