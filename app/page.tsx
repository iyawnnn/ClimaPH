...
export default function DashboardPage() {
  ...
  const getWeatherGreeting = () => {
    ...
    if (desc.includes("rain") || desc.includes("drizzle")) {
      return { line1: "Maulan ngayon!", line2: "Wag kalimutang magdala ng payong." };
    }
    if (desc.includes("sun") || desc.includes("clear")) {
      return { line1: "Mainit ang sikat ng araw!", line2: "Uminom ng maraming tubig." };
    }
    if (desc.includes("cloud")) {
      return { line1: "Maulap ngayon.", line2: "Magandang araw para mamasyal!" };
    }
    if (desc.includes("thunder") || desc.includes("storm")) {
      return { line1: "May badya ng kulog at kidlat.", line2: "Ingat po palagi!" };
    }
    ...
    return (
      <div className="flex h-screen w-full items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4 animate-pulse">
          <div className="w-14 h-14 rounded-full bg-primary/20 border border-primary/30"></div>
          <div className="w-48 h-6 rounded-md bg-muted"></div>
          <div className="w-32 h-4 rounded-md bg-muted"></div>
        </div>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="w-full flex-1 grid grid-cols-1 xl:grid-cols-[1fr_340px] 2xl:grid-cols-[1fr_440px] font-sans items-stretch min-h-full">
        ...
        <div className="flex flex-col min-w-0 bg-background p-4 md:p-6 lg:p-8 xl:p-10">
          <header className="w-full flex flex-col lg:flex-row lg:items-center justify-between gap-4 md:gap-6 shrink-0 mb-6 md:mb-8 xl:mb-10 mt-2 lg:mt-0">
            ...
            <div className="hidden lg:flex items-center gap-2 md:gap-3 w-full lg:max-w-md z-50 h-10 md:h-12" role="search">
              <div className="relative flex-1 h-full">
                <div className="h-full bg-card border border-border/60 shadow-sm rounded-xl overflow-hidden transition-all focus-within:ring-2 focus-within:ring-primary/20">
                  <SearchBar
                    input={input}
              ...
              <button
                ...
                disabled={isLocating}
                className="h-full aspect-square flex items-center justify-center rounded-full bg-primary text-primary-foreground hover:opacity-90 transition-all duration-200 shadow-md shrink-0 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary"
                aria-label="Detect and use current geographic location"
                aria-busy={isLocating}
              >
                <LocateFixed
                  className={`w-4 h-4 md:w-5 md:h-5 stroke-[2] ${isLocating ? "animate-spin text-secondary" : ""}`}
                  aria-hidden="true"
                />
              </button>
            </div>
          ...
          <main className="flex flex-col gap-4 md:gap-6 lg:gap-8 w-full" aria-label="Main Weather Dashboard">
            <div className="flex flex-col lg:flex-row gap-4 md:gap-6 w-full shrink-0 items-stretch">
              <section className="flex-1 min-w-0 rounded-[1.5rem] md:rounded-[2rem] bg-card border border-border/40 shadow-sm p-4 md:p-6 lg:p-8 min-h-[260px] md:min-h-[320px] lg:min-h-[380px] flex flex-col relative overflow-hidden transition-all hover:shadow-md" aria-labelledby="current-weather-heading">
              ...
              <section className="flex-1 min-w-0 rounded-[1.5rem] md:rounded-[2rem] bg-card border border-border/40 shadow-sm p-4 md:p-6 lg:p-8 min-h-[260px] md:min-h-[320px] lg:min-h-[380px] flex flex-col relative overflow-hidden transition-all hover:shadow-md" aria-labelledby="lifestyle-grid-heading">
            ...
            <div className="w-full shrink-0 flex flex-col md:flex-row gap-4 md:gap-6 lg:gap-10 mt-2 md:mt-4">
              <section className="flex-1 flex flex-col min-w-0 min-h-[160px] md:min-h-[200px]" aria-label="Hourly Forecast">
                <Suspense fallback={<Skeleton className="w-full h-full flex-1 rounded-xl" />}>
                  <ForecastCarousel />
                </Suspense>
              </section>

              <section className="w-full md:w-[220px] lg:w-[250px] xl:w-[275px] shrink-0 rounded-[1.5rem] md:rounded-[2rem] bg-card border border-border/40 shadow-sm p-4 md:p-6 flex flex-col justify-between items-center text-center hover:shadow-md transition-all min-h-[160px] md:min-h-[200px]" aria-label="Tomorrow Preview">
                <Suspense fallback={<Skeleton className="w-full h-full flex-1 rounded-xl" />}>
                  <TomorrowPreview />
                </Suspense>
              </section>
        ...
        <aside className="flex flex-col gap-6 lg:gap-8 bg-muted/40 border-t xl:border-t-0 xl:border-l border-border/30 p-4 pr-2 md:p-6 md:pr-4 lg:p-8 lg:pr-4 xl:p-10 xl:pr-6 h-full" aria-label="Secondary Weather Metrics">
          ...
          <section className="w-full shrink-0 min-h-[120px]" aria-label="UV Index">
            <Suspense fallback={<Skeleton className="w-full h-full flex-1 rounded-xl" />}>
              <UvIndexCard />
            </Suspense>
          </section>
